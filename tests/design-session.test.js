...
for (const mode of ['selectUrl', 'selectImage', 'capture', 'browse', 'url']) {
  test(`audit ${mode}: repeated sessions pass the actual supported-engine filter`, async () => {
    const {context: c, options} = sessionHarness();
    options.searchModeAction = mode;
    for (let round = 0; round < 2; round++) {
      const session = await c.createSession({engine: 'allEngines', sessionType: 'search', sessionOrigin: 'action'});
      assert.equal(session.searchMode, mode);
      const image = mode === 'url'
        ? {imageUrl: 'https://test.invalid/image.png', imageType: 'image/png'}
        : {imageUrl: 'https://test.invalid/image.png', imageType: 'image/png', imageDataUrl: 'data:image/png;base64,'};
      const supported = await c.getSupportedEngines(image, session.engines, mode);
      if (mode === 'url') {
        for (const site of localSites) assert.ok(!supported.includes(site), site);
      } else {
        assert.deepEqual(plain(supported), siteIds);
      }
      const tasks = await c.getSearches(image, supported, mode);
      assert.deepEqual(plain(tasks.map(x => x.engine)), plain(supported));
      assert.equal(new Set(tasks.map(x => x.engine)).size, tasks.length);
      for (const task of tasks.filter(x => localSites.includes(x.engine))) assert.equal(task.assetType, 'image');
    }
  });
}

test('Select URL needs downloadable bytes when any upload-only site is selected', async () => {
  const {context: c} = sessionHarness();
  assert.equal(await c.hasUrlSupport(siteIds), false);
  const failedDownload = {imageUrl: 'https://test.invalid/image.png', imageType: 'image/png'};
  const supported = await c.getSupportedEngines(failedDownload, localSites, 'selectUrl');
  assert.deepEqual(plain(supported), []);
});
...
test('eight added sites require file upload for URL images and conversion for WebP, GIF and AVIF', async () => {
  const {context: c} = sessionHarness();
  for (const type of [
    'image/png',
    'image/jpeg',
